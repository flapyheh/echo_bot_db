...
from app_bot.bot.infrastructure.database.db import get_user_banned_status_by_id
...
class ShadowbanMiddleware(BaseMiddleware):
    async def __call__(
        ...
        user_banned_status = get_user_banned_status_by_id(conn, user.id)
        if user_banned_status:
            if event.callback_query:
                event.callback_query.answer()
                return
        return await handler(event, data)